...
describe('libs/encryption', () => {
  describe('compare-text', () => {
    ...
    it("should throws an error if received hash isn't an string", async () => {
      expect(compareText(undefined as any, 'valid')).rejects.toThrow('Invalid hash value');
    });

    it("should throws an error if received text isn't an string", async () => {
      expect(compareText('valid', undefined as any)).rejects.toThrow('Invalid text value');
    });
  ...
  describe('hash-text', () => {
    it("should throws an error if received text isn't an string", async () => {
      expect(hashText(undefined as any)).rejects.toThrow('Invalid text value');
    });
